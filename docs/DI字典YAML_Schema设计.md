...
- `repeat` 支持三种方式指定重复次数：
  1. **`count`**: 固定次数（编译期已知），适用于固定重复结构，如：`count: 6`
  2. **`count_ref`**: 引用前面字段的解析结果作为重复次数（运行时动态）。引用时不要使用 `$` 前缀，直接使用字段 `id` 或 `name`。若被引用字段有 `id`，`count_ref` 应直接使用该 `id`
  3. **`count_expr`**: 通过表达式计算重复次数，如：`count_expr: "$remaining / 4"`
- 以上三种方式**互斥**，只能选择其中一种
