...
fn main() {
    ...
    let mut single = vec![0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
    show_json_case("主动上报状态字_with out event", 0x04001501, &single);

    single.push(0x01);
    show_json_case("主动上报状态字_with event count", 0x04001501, &single);
