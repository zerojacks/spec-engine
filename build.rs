...
fn gen_field(
    ...
    if let Some(cand) = &rf.candidate_ids {
        ...
        for idx in 0..count {
            ...
            repeated_rf.name = Some(format_build_repeat_name(
                &cand.name_template,
                Some(&generated_id),
                idx,
                count,
...
fn format_build_repeat_name(
    ...
    if let Some(template) = template {
        ...
        s = s.replace("{index0}", &idx.to_string());
...
fn format_id_expr(expr: &str, idx: usize) -> Result<String, String> {
    let value = eval_id_expr(expr, idx)?;
    Ok(format!("{:08X}", value))
}

fn eval_id_expr(expr: &str, idx: usize) -> Result<u32, String> {
    #[derive(Debug)]
    enum Token {
        Plus,
        Minus,
